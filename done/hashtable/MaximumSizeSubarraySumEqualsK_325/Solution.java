package LeetCode.done.hashtable.MaximumSizeSubarraySumEqualsK_325;
...
public class Solution {
    public int maxSubArrayLen(int[] nums, int k) {
        ...
        for(int i=0; i<nums.length; i++) {
            sum += nums[i];
            if (sum == k) max = i + 1; // subarray from the beginning to the current index till current position.
